...
The confirmed product model includes structuredInfo — fields the user answered directly (industry, categories, geographies, target_customers, regulated_entities, capabilities — several of these are lists, since a product can span more than one category, geography, customer segment, or regulated-entity relationship at once). Treat every value in structuredInfo as ground truth and never contradict it. If structuredInfo already answers something a clause is testing (e.g. a capability confirms a control exists), do not classify that as info_required — only classify as info_required when structuredInfo, the inferred elements, and discovery answers all fail to cover it.
